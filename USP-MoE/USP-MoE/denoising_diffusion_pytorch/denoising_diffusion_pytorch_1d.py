...
#from datapreparing import Encoder, Decoder

def decode_data(decoder, encoded_data,device):
    decoder.to(device)
...
class GaussianDiffusion1D(nn.Module):
    ...
    def p_sample(self, x, t: int, kgEmb, timeEmb, x_self_cond=None, clip_denoised=True):
        ...
        #pred_img = model_mean + (0.5 * model_log_variance).exp() * noise #更改系数
    ...
    @torch.no_grad()
    def ddim_sample(self, shape,kgEmb, timeEmb, clip_denoised=True):
    #def ddim_sample(self, shape, emb, clip_denoised=True):更改处！！！！！！！
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond,kgEmb, timeEmb, self_cond, clip_x_start=clip_denoised)
            #pred_noise, x_start, *_ = self.model_predictions(img, time_cond,emb, self_cond, clip_x_start=clip_denoised)更改处！！！！！！！
            if time_next < 0:
                img = x_start
                continue
    ...
    def sample(self, kgEmb, timeEmb, batch_size=16):  
        ...
        #sample_fn = self.p_sample_loop if self.is_ddim_sampling else self.ddim_sample
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample #更改处！！！！！！
        #pdb.set_trace()
                    ...
                    if self.step % self.save_and_sample_every == 0:  
                        ...
                        with torch.no_grad():
                            ...
                            for _ in range(self.sampleTimes):
                                if self.targetDataset == 'Orange_LSTM':  
                                ...
                                elif self.targetDataset in ['Marin','Orange_STGCN', 'Riverside','Marin_2','Placer_2','Riverside_2','shenzhen_4','ventura']:
                                    length = len(self.kgEmb)
                                    startNum = [0, length//10, 2*(length//10), 3*(length//10), 4*(length//10), 5*(length//10), 6*(length//10),7*(length//10), 8*(length//10), 9*(length//10), length]    
                                    generateNum = np.diff(startNum)
                                    result = None
                                    for id in range(len(generateNum)):
                                        smallkgEmb = self.kgEmb[startNum[id]:startNum[id+1]]
                            ...
                            #pdb.set_trace()
                            self.logger.info('params MAE: {}'.format(mae))
                            print('params MAE: {}'.format(mae))

                            self.writer.add_scalar('gene_params_mae', mae, self.step)

                            #params_8000 = np.load('/home/wll/G/Data/ModelParams_GWN_91872_8600_GUIYIHUA.npy')
                            #params_8000 = params_8000[-600:,:] 
                            #pdb.set_trace()
                            #params_out = np.load('/home/wll/G/GPD/Output/exp62/sampleRes_40040.npy')
                            #明天继续检查-----更改处
                            #print(self.scale)
                            #pdb.set_trace()
                            ...
                            # decoder = Decoder()
                            # decoder.load_state_dict(torch.load('/home/wll/G/GPD/decoder.pth'))
                            # sampleRes = sampleRes.reshape((sampleRes.shape[0],-1))
                            # sampleRes = sampleRes * self.scale
                            # sampleRes = torch.from_numpy(sampleRes).float()
                            # sampleRes = decode_data(decoder, sampleRes,device = device)
                            # sampleRes = sampleRes.cpu().numpy()

                            # metricsum = evalParams(sampleRes, self.config, device, self.logger, self.targetDataset, self.writer, self.step, self.basemodel)
