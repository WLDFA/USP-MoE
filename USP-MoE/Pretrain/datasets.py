...
class traffic_dataset2(Dataset):
    ...
    def load_data(self, stage, ifchosenode, test_data, nodeindex, ifspatial):  
        ...
        if stage == 'source':
            ...
            self.data_list = np.array([test_data])#只存储测试数据
        else:
        ...
        for dataset_name in self.data_list:
            if dataset_name=='BM' or dataset_name=='DC' or dataset_name=='man' or dataset_name=='bj':
                ...
                AAddr = self.data_args[dataset_name]['adjacency_matrix_path'] if ifchosenode==True else self.data_args[dataset_name]['adjacency_matrix_path']
            A, nodeset, initA = loadAM(AAddr, nodeindex)#提取节点对应的邻接矩阵 
            ...
            if dataset_name=='BM' or dataset_name=='DC' or dataset_name=='man' or dataset_name=='bj':
                ...
                XAddr = self.data_args[dataset_name]['dataset_path'] if ifchosenode==True else self.data_args[dataset_name]['dataset_path']
            #提取数据
            X = np.load(XAddr)
            #pdb.set_trace()
            # 生成随机数，维度为 2976*69*2
            # random_data = np.random.rand(2976, 140, 2)
