...
class D2STGNN(nn.Module):
    ...
    def __init__(self,model_args,**args):
        ...
        self.layers = nn.ModuleList([DecoupleLayer(fk_dim=self._forecast_dim, **model_args)])
        for _ in range(self._num_layers - 1):
            self.layers.append(DecoupleLayer(fk_dim=self._forecast_dim, **model_args))
    ...
    def _prepare_inputs(self, history_data):
        ...
        #pdb.set_trace()
        d = (history_data[:, :, :, num_feat] * self._model_args['tpd']).type(torch.LongTensor)
        time_in_day_feat = self.T_i_D_emb[d]
    ...
    def forward(self, history_data, label=None):  # (b, t, n, f)
        #pdb.set_trace()
        history_data = history_data.x #64*17*12*3
        # 生成随机数，维度为 64*6*12*2

        history_data = history_data.permute(0,2,1,3)
        ...
        history_data = self.embedding(history_data.float())
        ...
        forecast = torch.mean(forecast,dim=1)
        #forecast.transpose(1,2).contiguous().view(forecast.shape[0], forecast.shape[2], -1)
        return forecast
        #forecast.transpose(1, 2).unsqueeze(-1)


class DecoupleLayer(nn.Module):
    def __init__(self,fk_dim=256, **model_args):
        super().__init__()
        node_dim = model_args['node_hidden']
        self.estimation_gate = EstimationGate(node_emb_dim=node_dim, time_emb_dim=model_args['time_emb_dim'], hidden_dim=64)
        self.dif_layer = DifBlock(forecast_hidden_dim=fk_dim, **model_args)
        self.inh_layer = InhBlock(forecast_hidden_dim=fk_dim, **model_args)
...
class EstimationGate(nn.Module):
    ...
    def forward(self, node_embedding_u, node_embedding_d, time_in_day_feat, day_in_week_feat, history_data):
        batch_size, seq_length, num, _ = time_in_day_feat.shape
        #pdb.set_trace()
        f = node_embedding_u.unsqueeze(0).unsqueeze(0).expand(batch_size, seq_length,  num, -1)
        g = node_embedding_d.unsqueeze(0).unsqueeze(0).expand(batch_size, seq_length,  num, -1)
        estimation_gate_feat = torch.cat([time_in_day_feat, day_in_week_feat, f, g], dim=-1)
...
class DifBlock(nn.Module):
    def __init__(self, forecast_hidden_dim=256, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        super().__init__()
        adj_mx = np.load(model_args['adjacency_matrix_path'])
        adj_mx = self.normalize_adj_mx(adj_mx, 'doubletransition')
        adjs = [torch.tensor(i) for i in adj_mx]
        self.pre_defined_graph = adjs
        hidden_dim = model_args['num_hidden']
        self.localized_st_conv = STLocalizedConv(pre_defined_graph=self.pre_defined_graph, use_pre=use_pre, dy_graph=dy_graph, sta_graph=sta_graph, **model_args)

        self.forecast_branch = DifForecast(forecast_hidden_dim=forecast_hidden_dim, **model_args)
        self.backcast_branch = nn.Linear(hidden_dim, hidden_dim)
        self.residual_decompose = ResidualDecomp([-1, -1, -1, hidden_dim])
    
    
    def normalize_adj_mx(self,adj_mx, adj_type, return_type='dense'):
        if adj_type == 'normlap':
            adj = [self.calculate_asym_adj(adj_mx)]
        elif adj_type == 'doubletransition':
            adj = [self.calculate_asym_adj(adj_mx), self.calculate_asym_adj(np.transpose(adj_mx))]
    ...
    def calculate_asym_adj(self,adj_mx):
...
class STLocalizedConv(nn.Module):
    def __init__(self, pre_defined_graph=None, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        super().__init__()
...
class DifForecast(nn.Module):
    def __init__(self,forecast_hidden_dim=None, **model_args):
        super().__init__()
        self.k_t = model_args['k_t']
...
class InhBlock(nn.Module):
    def __init__(self,num_heads=4, bias=True, forecast_hidden_dim=256, **model_args):
        super().__init__()
        hidden_dim = model_args['num_hidden']
        ...
        self.forecast_block = InhForecast(forecast_hidden_dim, **model_args)
...
class InhForecast(nn.Module):
    def __init__(self, fk_dim, **model_args):
        super().__init__()
        self.output_seq_len = model_args['seq_len']
        self.model_args = model_args
        hidden_dim = model_args['num_hidden']
...
class DynamicGraphConstructor(nn.Module):
    ...
    def forward(self, **inputs):
        X = inputs['history_data'].float()
        E_d = inputs['node_embedding_d'].float()
        E_u = inputs['node_embedding_u'].float()
        T_D = inputs['time_in_day_feat'].float()
        D_W = inputs['day_in_week_feat'].float()
...
class DistanceFunction(nn.Module):
    ...
    def forward(self, X, E_d, E_u, T_D, D_W):
        ...
        c = self.bn(b)
        d = self.dropout(c)
        dy_feat = self.fc_ts_emb2(d)
        #dy_feat = self.fc_ts_emb2(self.dropout(self.bn(F.relu(self.fc_ts_emb1(X)))))
        dy_feat = dy_feat.view(batch_size, node_num, -1)

        emb1 = E_d.unsqueeze(0).expand(batch_size, node_num, -1)
        emb2 = E_u.unsqueeze(0).expand(batch_size, node_num, -1)
        #pdb.set_trace()
