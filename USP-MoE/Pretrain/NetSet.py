...
from Models.meta_lstm import *
from Models.agcrn import *
from Models.meta_d2stgnn import *
...
class StgnnSet(nn.Module): 
    ...
    """
    def __init__(self, data_args, task_args, model_args,model='v_GWN', node_num=207):
        ...
        if model == 'v_STGCN5':  # STGCN 5.0 which we choose
            ...
            self.model = AGCRN()    
        elif model == 'v_D2STGNN':
            self.model = D2STGNN(model_args)
        ...
        #优化器可转移使用，metr_lr学习率重要的超参数，weight_decay权重衰减
        ...
        self.loss_criterion = nn.MSELoss()#均方误差损失函数
    ...
    def calculate_loss(self, out, y, meta_graph, matrix, stage='target', graph_loss=True, loss_lambda=1):
        ...
        if graph_loss:
            ...
            loss = loss_predict + loss_lambda * loss_reconsturct#总损失
        else:
            loss = self.loss_criterion(out, y)
    ...
    def taskTrain(self, taskmode, target_dataloader, test_dataloader, target_epochs, logger, meanstd):
        ...
        for epoch in tqdm(range(target_epochs)):
            ...
            for step, (data, A_wave) in enumerate(target_dataloader):
                data, A_wave = data.cuda(), A_wave.cuda()
                data.node_num = data.node_num[0]
                #data.x是一个四维张量，其形状为(batch_size, node_num, seq_len, feature_dim)
                batch_size, node_num, seq_len, _ = data.x.shape
                hidden = torch.zeros(batch_size, node_num, self.model_args['hidden_dim']).cuda()
        ...
        with torch.no_grad():
            ...
            for step, (data, A_wave) in enumerate(test_dataloader):
                ...
                out = out * meanstd[1] + meanstd[0]#反归一化
                gold = data.y * meanstd[1] + meanstd[0]
    ...
    def node_taskTrain(self, modelname, node_index, init_index, target_dataloader, test_dataloader, train_meanstd, test_meanstd, target_epochs,logger, test_dataset, ifnewname, aftername):
        ...
        for epoch in tqdm(range(target_epochs)):
            ...
            for step, (data, A_wave) in enumerate(target_dataloader):
                ...
                elif self.model_name == 'v_STGCN5':
                    out, _ = self.model(data, A_wave[0].float())
                elif self.model_name == 'v_AGCRN':
                    adj_mx = [A_wave[0].float(), (A_wave[0].float()).t()]
                    out = self.model(data,adj_mx)
                elif self.model_name == 'v_LSTM':
                    adj_mx = [A_wave[0].float(), (A_wave[0].float()).t()]
                    out = self.model(data,adj_mx)
                elif self.model_name == 'v_D2STGNN':
                    adj_mx = [A_wave[0].float(), (A_wave[0].float()).t()]
                    out = self.model(data,adj_mx)
            ...
            if epoch % 5 == 0:
                with torch.no_grad():  
                    ...
                    for step, (data, A_wave) in enumerate(test_dataloader):
                        ...
                        data, A_wave = data.cuda(), A_wave.cuda()
                        #data = data*train_meanstd[1]+train_meanstd[0]
                        if self.model_name == 'v_GWN':
                            adj_mx = [A_wave[0].float(), (A_wave[0].float()).t()]
                            out = self.model(data, adj_mx)
                        elif self.model_name == 'v_STGCN5':
                    ...
                    # y_label = y_label.squeeze(2)
                    # y_label = y_label.mean(dim=1)
                    #pdb.set_trace()
                    outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
                    y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
    ...
    def node_taskTrain2(self, param, modelname, node_index, init_index, target_dataloader, test_dataloader, train_meanstd, test_meanstd, target_epochs,logger, test_dataset, ifnewname, aftername):
        ...
        if modelname == 'v_STGCN5':
            indexstart = [0, 256, 384, 512, 640, 1664, 2688, 3712, 7808, 11904, 16000, 16576]
            shapes = [(32, 8), (32, 1, 1, 4), (32, 1, 1, 4), (32, 1, 1, 4), (32, 8, 1, 4), 
                      (32, 8, 1, 4), (32, 8, 1, 4), (32, 32, 1, 4), (32, 32, 1, 4), (32, 32, 1, 4), (6, 96)]
        elif modelname == 'v_GWN':
            indexstart = [0, 2048, 4096, 6144, 8192, 10240, 12288, 14336, 16384, 18432, 20480, 
             22528, 24576, 26624, 28672, 30720, 32768, 33792, 34816, 35840, 36864, 
             37888, 38912, 39936, 40960, 41984, 43008, 44032, 45056, 46080, 47104, 
             48128, 49152, 49184, 49216, 49248, 49280, 49312, 49344, 49376, 49408, 
             49440, 49472, 49504, 49536, 49568, 49600, 49632, 49664, 54784, 59904, 
             65024, 70144, 75264, 80384, 85504, 90624, 90656, 91680, 91872]
            shapes = [(32, 32, 1, 2), (32, 32, 1, 2), (32, 32, 1, 2), (32, 32, 1, 2), 
                      ...
                      (32,), (32,), (32,), (32,), (32,), (32,), (32, 160, 1, 1), (32, 160, 1, 1), 
                      (32, 160, 1, 1), (32, 160, 1, 1), (32, 160, 1, 1), (32, 160, 1, 1), (32, 160, 1, 1), 
                      (32, 160, 1, 1), (32, 1, 1, 1), (32, 32, 1, 1), (6, 32, 1, 1)]   
        elif modelname == 'v_LSTM':
            indexstart = [0, 32, 64, 8256, 24640, 24896, 25152, 41536, 57920, 58176, 58432, 60480, 60512, 60704, 60710]
            shapes = [(32, 1, 1, 1), (32,), (256, 32), (256, 64), (256,), (256,), (256, 64), (256, 64), (256,), (256,), (32, 64), (32,), (6, 32), (6,)]

            # indexstart = [0, 256, 512, 768, 1024, 1280, 1536, 1792, 2048, 2304, 2560, 2816, 
            #               3072, 3328, 3584, 3840, 4096, 4224, 4352, 4480, 4608, 4736, 4864, 4992, 
            #               5120, 5248, 5376, 5504, 5632, 5760, 5888, 6016, 6144, 6160, 6176, 6192, 
            #               6208, 6224, 6240, 6256, 6272, 6288, 6304, 6320, 6336, 6352, 6368, 6384, 
            #               6400, 7040, 7680, 8320, 8960, 9600, 10240, 10880, 11520, 11536, 11792, 
            #               11888]
            # shapes = [(8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), 
            #           (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), 
            #           (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), (8, 16, 1, 2), 
            #           (8, 16, 1, 2), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), 
            #           (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), 
            #           (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), (16, 8, 1, 1), 
            #           (16, 8, 1, 1), (16, 8, 1, 1), (16,), (16,), (16,), (16,), (16,), (16,), (16,), 
            #           (16,), (16,), (16,), (16,), (16,), (16,), (16,), (16,), (16,), (16, 40, 1, 1), 
            #           (16, 40, 1, 1), (16, 40, 1, 1), (16, 40, 1, 1), (16, 40, 1, 1), (16, 40, 1, 1), 
            #           (16, 40, 1, 1), (16, 40, 1, 1), (16, 1, 1, 1), (16, 16, 1, 1), (6, 16, 1, 1)]
        index = 0
        ...
        for epoch in tqdm(range(target_epochs)):
            ...
            for step, (data, A_wave) in enumerate(target_dataloader):
                ...
                if self.model_name == 'v_GWN':
                    ...
                    out = self.model(data, A_wave[0].float())
